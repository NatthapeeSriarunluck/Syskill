  ...
  */
  ...
  {
    int xArray[8] = {-1, -1, -1, 0, 0, 1, 1, 1};
    int yArray[8] = {-1, 0, 1, -1, 1, -1, 0, 1};
    int count = 0;
    for (int i = 0; i < 8; i++)
    {
      int neighborX = row + xArray[i];
      int neighborY = col + yArray[i];
      if (is_alive(board, neighborX, neighborY))
      {
        count++;
      }
    }
  ...
  void make_next_board(life_board current, life_board next)
  {
    ...
    {
      for (int column = 0; column < current.num_cols; column++)
      {
        if (is_alive(current, row, column) && (count_live_nbrs(current, row, column) == 2|| count_live_nbrs(current,row,column) == 3))
        {
          set_alive(next, row, column);
        }
        else if (!is_alive(current, row, column) && (count_live_nbrs(current, row, column) == 3))
        {
          set_alive(next, row, column);
        }
        else
        {
          set_dead(next, row, column);
        }
      }
    }
  }
